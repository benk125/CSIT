...
GPIO.setup(tgreen, GPIO.OUT)
GPIO.setup(tred , GPIO.OUT)
GPIO.setup(tblue, GPIO.OUT)
...
TRED = GPIO.PWM(tred, Freq)
TGREEN = GPIO.PWM(tgreen , Freq)
TBLUE = GPIO.PWM(tblue, Freq)
...
if __name__ == "__main__":
    ...
    server.set_endpoint("opc.tcp://admin@192.168.43.40:4840/freeopcua/server/")
    server.set_server_name("FreeOpcUa Example Server")
    # set all possible endpoint policies for clients to connect through
    server.load_certificate("/home/pi/python-opcua/examples/my_cert.pem")
    server.load_private_key("/home/pi/python-opcua/examples/my_private_key.pem")
    ...
    dev.add_variable(0, "sensor1", "0").set_modelling_rule(True)
    dev.add_variable(0,"RGB_Sensor","0,0,0").set_modelling_rule(True)
    ...
    try:
        ...
        TBLUE.start(1)
        while (True):
            RGB = server.nodes.objects.get_children()[2]
            RGB_nn = RGB.get_children()[1]
            RGB_string = RGB_nn.get_data_value().Value.Value
            RGB_array = RGB_string.split(",") 
            temp_nn = RGB.get_children()[0]
            temp_val = int(temp_nn.get_data_value().Value.Value)
            if temp_val < 41:
                TBLUE.ChangeDutyCycle(0)
                TRED.ChangeDutyCycle(0)
                TGREEN.ChangeDutyCycle((temp_val/40)*100)
            if temp_val > 40 and temp_val < 81:
                TBLUE.ChangeDutyCycle((temp_val/80)*100)
                TRED.ChangeDutyCycle(0)
                TGREEN.ChangeDutyCycle(0)
            if temp_val > 80:
                TBLUE.ChangeDutyCycle(0)
                TRED.ChangeDutyCycle((temp_val/120)*100)
                TGREEN.ChangeDutyCycle(0)

            if RGB_array[1]:
                GREEN.ChangeDutyCycle(float(RGB_array[1]))
            if RGB_array[0]:
                RED.ChangeDutyCycle(float(RGB_array[0]))
            if RGB_array[2]:
                BLUE.ChangeDutyCycle(float(RGB_array[2]))
            time.sleep(0.5)
